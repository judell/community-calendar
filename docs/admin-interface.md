...
## 5. Health report as curator dashboard (question)

The health report (`report.html`) is evolving into the primary tool curators use to monitor their calendar. Currently it's a static page generated by the CI pipeline. Should it become a third section of the curator's working environment alongside `SOURCES_CHECKLIST.md` and the calendar UI itself?

The pattern that emerged: a curator sees `volunteer_toronto` at 0 events in the dashboard, cares about that category, tasks an agent to investigate, and the agent finds an alternative source. The dashboard surfaced the problem; the curator's judgment drove the response.

Questions to consider:
- Should the dashboard be interactive (click a zero-event feed to start an investigation)?
- Should it link to `SOURCES_CHECKLIST.md` entries for each feed?
- Should it support curator annotations ("known issue", "investigating", "replaced by X")?
- Is it the right place to surface category gaps (e.g., "no volunteer sources"), not just feed failures?
