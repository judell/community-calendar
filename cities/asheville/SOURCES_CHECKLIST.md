...
| Wortham Center for the Performing Arts | worthamarts.org | TBD | WordPress custom (11+ events, Diana Wortham Theatre, 500 cap). No Tribe/MEC; own box office. Check Songkick. |
...
| Asheville Chamber of Commerce | web.ashevillechamber.org/cwt/external/wcpages/wcevents/events_communitystartpage.aspx | RSS scraper | Older ChamberMaster/GrowthZone (ASP.NET, not modern REST). RSS feed at `?rss=true` returns 30 events (networking, ribbon cuttings, community events). Modern `/api/events` endpoint returns 404. Needs RSS-to-ICS scraper. |
...
| Buncombe County Libraries | LibraryCalendar proprietary platform (buncombe.librarycalendar.com); no ICS at `/events/feed`, `/events/ical`, or any standard path; `/events/feed/html?_wrapper_format=lc_calendar_feed` returns HTML not ICS; closed system with no public API. Note: county gov calendar at buncombenc.gov is a separate CivicPlus system that *does* have ICS feeds (see Currently Implemented). |
