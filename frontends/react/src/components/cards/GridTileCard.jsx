...
export default function GridTileCard({ event, filterTerm, onCategoryFilter }) {
  ...
  return (
    <>
      <div
        className="aspect-square relative rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-200 cursor-pointer group"
        onClick={() => setShowDetail(true)}
      >
        {/* Background */}
        {event.image_url ? (
          <img src={event.image_url} alt="" className="absolute inset-0 w-full h-full object-cover" loading="lazy" onError={hideOnImgError} />
        ) : (
          <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${colors.label}20, ${colors.background})` }}>
